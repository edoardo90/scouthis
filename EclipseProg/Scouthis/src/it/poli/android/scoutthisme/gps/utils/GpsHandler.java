...
{	
	...
	private Timer timer;
	private TimerTask gpsCheckAgeTask;
	...
	{	
		...
		if (view != null && container != null && gpsAlertFragment != null) {
			if (container.getChildCount() > 0) {
				...
				if (mFragment instanceof FindFriendsLoggedFragment) {
					TextView txtStatus = (TextView) view.findViewById(R.id.txtFindFriendsStatus);
					txtStatus.setText(Html.fromHtml(mFragment.getString(R.string.generic_status_waiting_gps)));
				}
			}
		}
	}

	private void schedTimerTask (boolean timerActivated)
	{
		if (timerActivated) {
			timer = new Timer();
			final Handler handler = new Handler();
			gpsCheckAgeTask = new TimerTask() {       
				@Override
				public void run() {
					handler.post(new Runnable() {
						public void run() { 
							if (lastLocation != null) {
								if (ageMilliseconds(lastLocation) >= Constants.GPS_LAST_UPDATE_MILLISECONDS) {
									lastLocation = null;
			...
			timer.schedule(gpsCheckAgeTask, 0, Constants.TIME_UPDATE_GPS_STATUS);
		} else if (timer != null)
			timer.cancel();
	}
